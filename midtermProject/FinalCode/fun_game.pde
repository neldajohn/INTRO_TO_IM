...
SoundFile soundfile; //knock sound
SoundFile gameSoundfile; //background sound 
SoundFile lostwordfile; //lost words sound

//GLOBAL VARIABLES
int game_score;  //general game score
int blocks_left; //no. of blocks left
int numberOfWords; //number of words in the game
int total_points; //total number of points


Word[] myWordsArray = new Word[71]; //array of words
ArrayList<Integer> myPositions = new ArrayList<Integer>(); //array of scored words' positions
ArrayList<Integer> lostPositions = new ArrayList<Integer>(); //array of lost words' positions
ArrayList<Word> myScoredWords = new ArrayList<Word>(); //array of scored words

String words[]; //words array when they are taken out of the csv file
String background; //background color

Level game; //game level object
boolean gameProceed; //flag on whether the game is proceeding
...
void setup()
{
  ...
  if (game.gameEnd != true) //if the game has not ended
  {
    gameSoundfile.loop(); //play the background sound
  }
  ...
  for (int i = 0; i < words.length; i++)
  {
    ...
    float my_speed;
    if (my_yPos > -2000 ) //slower speed for words that are closer
    {
      my_speed = random (1.0, 2.0);
    } else if ( my_yPos > -4000 && my_yPos < -2000) //middle words with average speed
    {
      my_speed = random (2.0, 3.5);
    } else
    {
      my_speed = random(3.5, 5.0);//fast speed for words that are further
    }
    
    //make sure that words are within the canvas
    if (my_xPos > width - 10*(words[i].length()))
    {
      my_xPos = (i-20) *17;
    }
    
    //create a new word object and add it to myWordsArray
...
class Player
{
  ...
  void showPlayer()
  {
    ...
    //load the player's image and display it
  ...
  boolean Knock()
  {
    ...
    {
      float y_knockPoint = (myWordsArray[i].y_pos + myWordsArray[i].word_height); //y position for knock
      float x_knockPoint = (myWordsArray[i].x_pos + myWordsArray[i].word_width); //y position for knock
      if ((y_knockPoint >= y_pos - pRadius) && (y_knockPoint <= y_pos + pRadius) && (x_pos+pRadius >= myWordsArray[i].x_pos) && (x_pos - pRadius <= x_knockPoint ) )
      {
        boolean scored = false; //check if a word has been scored or not
        for (int num : myPositions)
        {
          if ( i == num)
          {
            scored = true; //set to true if scored
          }
        } 
        if (scored == false) //unless not scored, do not add to the game score
        {
  ...
  boolean LostWords()
  {
    ...
    for (int i = 0; i < myWordsArray.length; i++)
    {
      ...
      {
        boolean lost = false; //check if a word has been lost or not
        for (int num : lostPositions)
        {
          if (i == num)
          {
            lost = true; //set to true if it goes beyond the platform
          }
        }
        if (lost == false)
        {
          if (blocks_left > 0)
          {
            blocks_left -= 1; //decrease num of blocks left
            lostPositions.add(i); //add position to lostPositons
            lostwordfile.play(); //play sound
...
class Word
{
  ...
  void update()
  {
    if (y_pos < height)
    {
      y_pos += word_speed; //increase speed
    }
  }
  ...
  void displayWord()
  {
    //display the word  in a box depending on the level
    if (background == "blue")
    {
      fill(78, 182, 237);
...
class Level
{
  ...
  {
    my_circle.LostWords();
    if (my_circle.alive == true) //if the player is alive
    {
      return false;
    } else
    {
    ...
    } else if (gameStart == true)
    {
      ...
      if (!(checkWin()) && !(checkLoss()))
      {
        ...
        for (int i = 0; i < myWordsArray.length; i++)
        {
          ...
          if (scored == false) //only show if not scored
          {
        ...
        my_platform.showPlatform(); //show platform
        
        //check for knocks, lost words, or if the game is won
        my_circle.Knock();
        my_circle.LostWords();
        my_circle.WonGame();

        //show score and no. of blocks left
...
//show the start page cover
void showCover()
{
   //boxes for the main "WORD GAME" and levels
...
//Function to check what level was picked
...
void mouseClicked()
{
  if (game.gameStart == false) //if the game hasn't started
  {
